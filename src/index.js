...
async function handleRequest(request) {
  ...
  try {
    ...
    // Fetch M3U8 to extract audio tracks
...
#player{width:100%;height:100%;position:relative;display:flex;justify-content:center;align-items:center;}
...
if(Hls.isSupported()){
  ...
  audioTracks.forEach(track=>{
    if(track.uri){
      ...
      option.text = track.name + (track.language ? " ("+track.language+")" : "")
